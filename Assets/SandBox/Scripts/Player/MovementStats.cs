...
public class MovementStats : MonoBehaviour
{
    ...
    [HideInInspector] public bool GrabLedgeAirCheckStatus;
    [HideInInspector] public bool GrabLedgeGroundCheckStatus;
    ...
    {
        if (collision.otherCollider != TargetCollider) return;
    ...
    void RefreshCanGrabLedge()
    {
        ...
        bool airCheck = CheckOverlap(airCheckRect, WhatIsGround, false);
        bool groundCheck = CheckOverlap(groundCheckRect, WhatIsGround, true);
        CanGrabLedge = !airCheck && groundCheck;
        GrabLedgeAirCheckStatus = !airCheck;
        GrabLedgeGroundCheckStatus = groundCheck;
    }
    ...
    private bool CheckOverlap(Rect area, LayerMask mask, bool noTriggers)
    {
        if (!noTriggers)
            return Physics2D.OverlapArea(area.min, area.max, mask);
        
        int count = Physics2D.OverlapAreaNonAlloc(area.min, area.max, _buffer, mask);
        return _buffer.Take(count).Any(c => !c.isTrigger);
    }
    ...
    private void DrawGroundCheck()
    {
        ...
        Gizmos.color = CheckOverlap(groundCheck, WhatIsGround, false) ? Color.magenta : Color.white;
    ...
    private void DrawGrabLedgeChecks()
    {
        ...
        Gizmos.color = !CheckOverlap(airCheck, WhatIsGround, false) ? Color.magenta : Color.white;
        GizmosEx.DrawRect(airCheck);

        Gizmos.color = CheckOverlap(groundCheck, WhatIsGround, true) ? Color.magenta : Color.white;
        GizmosEx.DrawRect(groundCheck);
    }
