...
public class Throw : MonoBehaviour
{
   ...
   // private PlayerClimb playerClimb;
    ...
    public SpriteRenderer pointerKnife;
    ...
    void Start()
    {
        ...
        playerClimbX = GetComponentInParent<PlayerClimbX>();
...
//        animator.SetFloat("AngleToThrow", angle);
        ...
        transform.right = direction;
            ...
            if (Input.GetMouseButtonUp(0) && !player.isDead  && canBeUsedKnives) //&& !playerHook.isHooked
            {
            ...
            if (Input.GetMouseButtonUp(1) && !player.isDead && !playerClimbX.isHanging && canBeUsedWater)
            {
                if (!GameManager.gameIsPaused)
                {
    ...
    void InputCheck()
    {
        ...
        if (Input.GetMouseButton(0) && canBeUsedKnives && amountOfKnives > 0  && !player.isDead && !playerClimbX.isHanging)
        {
            pointerKnife.enabled = true;
        }

        else if (Input.GetMouseButton(1) && canBeUsedWater && amountSaintWater > 0  && !player.isDead && !playerClimbX.isHanging)
        {
            pointerKnife.enabled = true;
        }
        else if (Input.GetKey(KeyCode.R)  && !player.isDead && !playerClimbX.isHanging && !abilities.IsGrenadeUSAGECoolingDown)
        {
            pointerKnife.enabled = true;
        }
        else
    ...
    public void ShootGrenade()
    {
       ...
       // saintWaterCounter.amount = amountSaintWater;
       // saintWaterCounter.TextUpdate();
       abilities.IsGrenadeUSAGECoolingDown = true;
       abilities.WithdrawStamina(abilities.grenadeCost);
    ...
    public void DirectionCheckGrenade()
    {
        animator.SetBool("isThrowing", true);
        if (player.facingRight)
        {
            if (angle < 90f && angle > 45f)
            {
                animator.Play("ThrowAGrenadeUp");
            }
            else if (angle < 45f && angle > -45f)
            {
                animator.Play("ThrowAGrenadeRight");
            }
            else if (angle < -45f && angle > -90)
            {
                animator.Play("ThrowAGrenadeDown");
            }
            else if (angle < -90 && angle > -135)
            {
                player.Flip();
