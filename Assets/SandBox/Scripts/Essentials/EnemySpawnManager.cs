...
public class EnemySpawnManager : MonoBehaviour
{
    ...
    public float LavaEdgeOffset = 18f;
    ...
    void SpawnReady()
    {
        ...
        var piece = enemySpawners.OrderBy(a => Random.Range(0, 1000)).Where(p => !IsPortalInLava(p)) // CAN CAUSE SHIT
            .Take(enemiesOnWaves[currentWave]).ToArray();
    ...
    bool IsPortalInLava(Transform portal)
    {
        var lava = FindObjectOfType<LavaUprising>();

        if (lava)
            return portal.transform.position.y < (lava.transform.position.y + LavaEdgeOffset);
